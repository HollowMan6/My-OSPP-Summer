...
![ivfflat索引结构.svg](imgs/ivfflat索引结构.svg)
...
![hnsw索引检索.png](imgs/hnsw索引检索.png)
...
https://github.com/oceanbase/oceanbase/blob/vector_search/docs/vector-search-manual.md 
  - [X] 根据设计方案完成编码工作。
  - [X] 通过已有mysqltest测试集。
  - [ ] 编写并完善可验证IVFPQ索引功能的mysqltest测试用例。
  - [ ] 进一步优化IVFPQ索引的性能，包括但不限于：
    - [ ] 优化IVFPQ索引的构建过程，减少构建时间；
    - [ ] 优化IVFPQ索引的查询过程，减少查询时间；
    - [ ] 优化IVFPQ索引的内存占用，减少内存占用。

- [ ] 第11-13周 (9.9 - 30, 2024) 共计3周
